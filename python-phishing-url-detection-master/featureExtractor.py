...
def featureExtraction(url):
    ...
    if response:  
        ...
        # If there is no response, append default values or zeros
        dom.extend([0, 0, 0])  # Assuming default values are 0 for these features
    ...
    # Ensure pca.transform is applied correctly with the right shape
    features.append(pca.transform(dom_pd)[0][0])

    # Creating a DataFrame for the row of features
    row = pd.DataFrame([features], columns=feature_names)
